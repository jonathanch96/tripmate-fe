...
export function AuthShell({ title, description, alternate, children }: {
  ...
  return (
    <main className="flex min-h-dvh flex-col bg-[oklch(0.24_0.045_255)] lg:grid lg:grid-cols-[minmax(320px,42%)_1fr]">
      <div className="relative flex min-h-[238px] flex-col justify-between overflow-hidden bg-[oklch(0.24_0.045_255)] p-6 text-white lg:min-h-0 lg:p-14">
        ...
        <div className="relative max-w-[400px]">
          <h1 className="font-heading text-[30px] leading-[1.15] font-extrabold lg:text-[38px]">
            Travel together.
            <br />
            Split easily.
          </h1>
          <p className="mt-3 hidden text-[15px] leading-relaxed text-white/80 sm:block lg:mt-4">
            Track shared expenses across currencies, split any way you like, and always know who owes who.
          </p>
        </div>
        <p className="relative hidden text-[13px] text-white/50 lg:block">© {new Date().getFullYear()} TripMate</p>
      </div>
      <div className="relative -mt-5 flex w-full flex-1 items-start justify-center rounded-t-[28px] bg-background px-6 py-9 lg:mt-0 lg:items-center lg:rounded-none lg:py-12">
