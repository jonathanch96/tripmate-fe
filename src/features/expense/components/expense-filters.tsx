...
export function ExpenseFilters({ participants, currencies, categories }: { participants: Participant[]; currencies: string[]; categories: ExpenseCategory[] }) {
  ...
  const filterFields = <>
    ...
    <NativeSelect aria-label="Member" value={current.get("split_user_id") ?? ""} onChange={(event) => set("split_user_id", event.target.value)} className="min-w-[128px]"><NativeSelectOption value="">All members</NativeSelectOption>{participants.map((participant) => <NativeSelectOption key={participant.userId} value={participant.userId}>{participantName(participant)}</NativeSelectOption>)}</NativeSelect>
  ...
  return <>
    <div className="space-y-2.5 md:hidden">
      <Input aria-label="Search expenses" placeholder="Search expenses" defaultValue={current.get("q") ?? ""} onBlur={(event) => set("q", event.target.value)} className="h-12 rounded-[12px] bg-white" />
      <details className="rounded-[12px] border border-border bg-white p-3">
        <summary className="cursor-pointer text-sm font-bold">Filters</summary>
        <div className="mt-3 grid grid-cols-2 gap-2 [&>*]:min-w-0">{filterFields}</div>
      </details>
    </div>
    <div className="hidden flex-wrap items-center gap-2 rounded-xl border p-3 md:flex">
