...
export function ExpenseDialog({ trip, participants, expense, pending, open: controlledOpen, onOpenChange, onSubmit }: { trip: Trip; participants: Participant[]; expense?: Expense; pending: boolean; open?: boolean; onOpenChange?: (open: boolean) => void; onSubmit: (payload: ExpensePayload) => void }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
      ...
      <DialogFooter><Button disabled={!valid || pending} onClick={() => { onSubmit(expenseCreateSchema.parse(payload)); setOpen(false) }}>{pending ? "Saving…" : expense ? "Save changes" : "Save expense"}</Button></DialogFooter>
    </DialogContent>
  </Dialog>
}
