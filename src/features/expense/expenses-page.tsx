...
export function ExpensesPage() {
  ...
  return <section className="space-y-5"><div className="flex items-center justify-between"><div><h2 className="font-heading text-xl font-semibold">Expenses</h2><p className="text-sm text-muted-foreground">Record each bill once, even when several people paid.</p></div><ExpenseDialog trip={trip} participants={participants} pending={create.isPending} onSubmit={(payload) => create.mutate(payload)} onReceiptConverted={refresh} /></div>{editing ? <ExpenseDialog key={editing.id} trip={trip} participants={participants} expense={editing} open onOpenChange={(open) => { if (!open) setEditing(null) }} pending={update.isPending} onSubmit={(payload) => update.mutate({ expense: editing, payload })} /> : null}<ExpenseFilters participants={participants} currencies={currencies} categories={categories.data ?? []} />{query.isLoading ? <p className="text-muted-foreground">Loading expenses…</p> : <ExpenseTable expenses={expenses} categories={categories.data ?? []} pendingAction={action.isPending || update.isPending} onEdit={setEditing} onDelete={(expense) => action.mutate({ expense, action: "delete" })} onApprove={(expense) => action.mutate({ expense, action: "approve" })} onReject={(expense) => { const reason = window.prompt("Why is this expense rejected?"); if (reason) action.mutate({ expense, action: "reject", body: { reason } }) }} />}</section>
}
