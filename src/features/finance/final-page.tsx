...
import { opposingRate, parsePair, type RateDraft } from "@/features/finance/rate-pair-helpers"
...
export function FinalPage() {
  ...
  return <section className="space-y-6"><div className="flex items-center justify-between"><div><h2 className="font-heading text-xl font-semibold">Final settlement</h2><p className="text-sm text-muted-foreground">Lock rates and finish with the smallest practical transfer plan.</p></div>{trip.isFinalized ? <Badge>Finalized</Badge> : null}</div>
    <Card id="rates"><CardHeader><CardTitle>Exchange rates</CardTitle></CardHeader><CardContent className="space-y-3">{rates.isLoading ? <LoadingState label="Loading rates…" /> : rates.data?.map((rate) => <div key={rate.id} className="flex items-center justify-between rounded-lg border p-2.5 text-sm"><span>{rate.from} → {rate.to}</span><span className="flex items-center gap-2 tabular-nums">{rate.rate} {rate.isFinal ? <Badge variant="outline">locked</Badge> : null}</span></div>)}
      ...
      {trip.canEditSettings && !trip.isFinalized ? <div className="space-y-2">{drafts.map((draft, index) => <div key={index} className="space-y-1"><form className="grid gap-2 sm:grid-cols-4" method="post" onSubmit={(e) => { e.preventDefault(); setRate.mutate(draft) }}>
        ...
        <Button disabled={setRate.isPending} type="submit">{setRate.isPending ? <><Spinner className="mr-1.5" />Setting…</> : "Set & lock rate"}</Button>
      </form>
    ...
    {plan.error ? <MissingRateState error={plan.error} tripCode={trip.code} /> : <Card><CardHeader><CardTitle>Optimized transfer plan</CardTitle></CardHeader><CardContent className="space-y-4">{plan.isLoading ? <LoadingState label="Calculating plan…" /> : plan.data?.transfers.length ? plan.data.transfers.map((transfer, i) => <div key={i} className="rounded-lg border p-3"><div className="flex justify-between text-sm"><span>{names.get(transfer.fromUserId)} → {names.get(transfer.toUserId)}</span><strong className="tabular-nums">{formatMoney(transfer.amount, transfer.currency)}</strong></div>{transfer.bankAccountNumber ? <p className="mt-2 text-sm text-muted-foreground">{transfer.bankName} · {transfer.bankAccountNumber} · {transfer.bankAccountHolder}</p> : null}</div>) : <div className="rounded-lg border border-dashed p-6 text-center text-muted-foreground">Everyone is settled up. No transfers are needed.</div>}</CardContent></Card>}
    {trip.canEditSettings ? <div className="flex justify-end">{trip.isFinalized ? <Button variant="outline" disabled={finalize.isPending} onClick={() => finalize.mutate("unfinalize")}>{finalize.isPending ? <><Spinner className="mr-1.5" />Reopening…</> : "Unfinalize trip"}</Button> : <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
      <AlertDialogTrigger render={<Button />} disabled={finalize.isPending || Boolean(plan.error)}>{finalize.isPending ? <><Spinner className="mr-1.5" />Finalizing…</> : "Finalize trip"}</AlertDialogTrigger>
